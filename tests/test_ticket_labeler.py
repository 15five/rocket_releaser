...
def test_no_jira_ticket_mentioned(mocker: MockFixture):
    ...
    t = TicketLabeler('', pull_request_dicts, '', '', 'fakeJiraToken', 'bob@company.com', 'https://foo.atlassian.net')
...
def test_one_jira_tickets_with_number_in_title(mocker: MockFixture):
    pull_request_dicts = [{
        'number': 5,
        'title': 'removing middle finger emoji [ENG-666]'
    }]
...
def test_multiple_jira_tickets_with_number_in_body(mocker: MockFixture):
    pull_request_dicts = [{
        ...
        'number': 6,
        'title': 'More foobar',
        'body': 'More foobar closes [ENG-667]'
    }]
...
def test_one_jira_referenced_in_both_title_and_body_only_returns_single_issue(mocker: MockFixture):
    pull_request_dicts = [{
        'number': 6,
        'title': 'More foobar [ENG-667]',
        'body': 'closes ENG-667'
