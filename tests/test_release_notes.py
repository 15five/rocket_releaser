...
def mock_reqs(mocker):
    ...
    mocker.patch('prs.GraphQL')
    mocker.patch('slack.slacker')
...
bla bla bla
    ...
    mocker.patch('prs.PRs.pull_request_dicts', return_value=[mock_pr_1])
    slack_text = release_notes.release_notes('github_token', 'jira_token', '0782415', '8038fc3', 'slack_webhook_key',
                                             'env_name', 'vpc_name', 'repo_dir', 'master', 'org_name', 'repo_name', jira_username='',
                                             jira_url = '', dry_run=True)
    assert '0 PRs found' in slack_text
...
bla bla bla
    ...
    changelog_str = release_notes.turn_changelog_into_string([mock_pr_1], 'preview', '0782415', '8038fc3', 1, 'org_name', 'repo_name')
...
def test_turn_changelog_into_string_detects_hotfix():
    ...
    changelog_str = release_notes.turn_changelog_into_string([mock_pr_1], 'production', '0782415', '8038fc3', 1, 'org_name', 'repo_name')
...
def test_turn_changelog_into_string_detects_when_commits_are_same():
    changelog_str = release_notes.turn_changelog_into_string([], 'preview', '', '', 55, 'org_name', 'repo_name')
...
bla bla bla
    ...
    changelog_str = release_notes.turn_changelog_into_string([mock_pr_1], 'preview', '', '', 55, 'org_name', 'repo_name')
