...
def turn_changelog_into_string(pull_request_dicts: List[dict], env_name: str, from_revision: str, to_revision: str,
                               num_jira_tickets: int, org_name: str, repo_name: str):
    ...
    is_hotfix = any(['hotfix' in pr.get('title', '').lower() and env_name.lower() in pr.get('title', '').lower()
    ...
    if from_revision == to_revision:
        messages.append('Start and end commit hashes are the same - no changes for release notes to log')
        return '\n'.join(messages)
...
def release_notes(github_token: str, from_revision: str, to_revision: str, slack_webhook_key: str, env_name: str, vpc_name: str,
                  repo_dir: str, search_branch: str, org_name: str, repo_name: str, jira_token: str = '', jira_username: str = '',
                  jira_url: str = '', label_tickets=True, verbose=False, dry_run=False, fetch_before=True):

    if not repo_dir:
        repo_dir = get_default_repo_dir()

    logger.info(f'Pulling deploy SHAs from {search_branch} branch in {repo_dir}. {from_revision}...{to_revision}')
    deploy_shas = SHAs(repo_dir, fetch_before).get_shas(from_revision, to_revision, branch=search_branch)
    ...
    pull_request_dicts = [pr for pr in prs.pull_request_dicts(deploy_shas) if pr['merged']]

    num_jira_tickets: int = 0

    if label_tickets:
        ticket_labeler = TicketLabeler(github_token, pull_request_dicts, org_name, repo_name,
                                       jira_token, jira_username, jira_url)
        num_jira_tickets = ticket_labeler.label_tickets(env_name, vpc_name, dry_run=dry_run)
        logger.info(f'labeled {num_jira_tickets} tickets')

    slack_text = turn_changelog_into_string(pull_request_dicts, env_name, from_revision, to_revision, num_jira_tickets, org_name, repo_name)
...
def main(args: List[str]):
    ...
    parser.add_argument('-r', '--repo_dir',
                        ...
                        default='')
    parser.add_argument('-b', '--search_branch',
                        help='Default is "dev"',
                        default='dev')
    parser.add_argument('-e', '--env_name',
                        help='Eg. "preview"',
                        default='preview')
    parser.add_argument('-V', '--vpc_name',
                        help='Eg. "cloud100"',
                        default='cloud100')
    parser.add_argument('-l', '--dont_label_tickets',
    ...
    parser.add_argument('--jira_username',
                        help='Eg. "bob@company.com"',
                        default='')
    parser.add_argument('--jira_url',
                        help='Eg. "https://company.atlassian.net"',
                        default='')

    parsed_args = parser.parse_args(args)

    logger.info('Running release notes script with following options: ' + str({
