...
class ChangeLog:
    ...
    def __init__(self, pull_request_dicts: List[dict], org_name: str, repo_name: str, jira_url=''):
    ...
    def release_bodies(self):
        return [(pr.get('number'), pr.get('body')) for pr in self.pull_request_dicts
    ...
    def add_jira_link(line: str, jira_url: str):
        return re.sub(r'\[(\w+-\d+)\]', lambda match: f'<{jira_url}/browse/{match.group(1)}|{match.group(1)}>', line)

    @staticmethod
    def linkify(org_name: str, repo_name: str, line: str, pr_number, jira_url=''):
        """add jira & github links"""

        if jira_url: line = ChangeLog.add_jira_link(line, jira_url)
    ...
    def parse_bodies(self):
        for pr_number, body in self.release_bodies:
            ...
            if fixes:
                self.fixes.append(self.linkify(self.org_name, self.repo_name, fixes, pr_number, self.jira_url))
            if noteworthy:
                self.noteworthy.append(self.linkify(self.org_name, self.repo_name, noteworthy, pr_number, self.jira_url))
            if features:
                self.features.append(self.linkify(self.org_name, self.repo_name, features, pr_number, self.jira_url))
            ...
            if qa_notes:
                self.qa_notes.append(self.linkify(self.org_name, self.repo_name, qa_notes, pr_number, self.jira_url))
